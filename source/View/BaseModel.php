...
abstract class Silva_View_BaseModel extends Silva_View
{
	...
	{
	    //trace("BaseModel default options");
	...
	public function editModel($returnPartial = true)
	{
	    ...
	    if ($activeRecord->isNew()) {
	        ...
	        if ($this->catRelationMap !== null) {
	            //trace($this->getCategoryLocalReferencePhpName());
	            //trace($this->getCategoryForeignReferenceName());
	            //trace($_GET[$this->getCategoryForeignReferenceName()]);
	            ...
	            //trace($activeRecord->{"get{$this->getCategoryLocalReferencePhpName()}"}());
	        }
	    }
	    ...
	    if ($this->hasI18nBehavior()) {
	        $i18nProperties = $this->getI18nBehavior();
	        $this->locale = isset($_GET[Silva_Backend::URL_QUERY_LOCALE]) ? $_GET[Silva_Backend::URL_QUERY_LOCALE] : $i18nProperties['default_locale'];
	        //trace('if ($this->hasI18nBehavior())..., locale:'. $this->locale);
	        $activeRecord->setLocale($this->locale);
	        //trace('locale of $activeRecord:' . $activeRecord->getLocale());
	    }

	    //trace('before call to __getActiveRecordForm, locale of activeRecord: ' . $activeRecord->getLocale());
	    $form = $this->getActiveRecordForm($activeRecord);
	    //trace('after call to __getActiveRecordForm, locale of activeRecord: ' . $activeRecord->getLocale());
	...
	protected function saveActiveRecord($activeRecord, $form)
	{
	    //trace(__METHOD__);
	    if ($this->options['autoBuildForm']) {
	        //trace("locale: " . $activeRecord->getLocale());
	...
	protected function getSilvaForm($modelInstance, array $actionQuery = array())
	{
	    //trace(__METHOD__);
	    //trace($this->options);
	    //trace('locale of $modelInstance:' . $modelInstance->getLocale());
	    ...
	    //trace('locale of $this->locale:' . $this->locale);
